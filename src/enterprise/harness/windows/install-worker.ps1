...
$launcherPattern = ([regex]::Escape("$Source\src\enterprise\harness\windows\run-worker.py") + "|" + [regex]::Escape("$Source\windows\run-worker.py"))
if (Get-CimInstance Win32_Process | Where-Object { $_.Name -in @('python.exe','pythonw.exe') -and $_.CommandLine -match $launcherPattern }) {
    throw 'A worker process still exists. Stop the idle worker with src/enterprise/harness/windows/stop-worker.ps1 first.'
}
...
$launcher = "$Source\src\enterprise\harness\windows\run-worker.py"
