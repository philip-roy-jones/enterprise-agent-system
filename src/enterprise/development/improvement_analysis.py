...
def inspect_library(root):
    root = Path(root)
    source = (root / "src/enterprise/workflows/finance/procedures.py").read_text()
    ...
    graph_source = (root / "src/enterprise/workflows/finance/graph.py").read_text()
    ...
    return {
        ...
        "source_hashes": {
            "src/enterprise/workflows/finance/procedures.py": hashlib.sha256(source.encode()).hexdigest(),
            "src/enterprise/workflows/finance/graph.py": hashlib.sha256(graph_source.encode()).hexdigest(),
        },
    }
...
def patch_library(path, analysis):
    ...
    if (
        ...
        != analysis["library"]["source_hashes"]["src/enterprise/workflows/finance/procedures.py"]
    ):
...
import pytest
from enterprise.workflows.finance.procedures import AMOUNT_LABELS, GRAPH_VERSION, resolve_amount_label
