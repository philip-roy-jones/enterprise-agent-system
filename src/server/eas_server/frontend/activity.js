...
const activityView = (() => {
  ...
  function summary(event) {
    ...
    if (event.kind === "action_started") return `${d.action.name} · ${d.invocation.includes(":workflow:") ? "internal workflow step" : "direct operation"}`;
    if (event.kind === "action_result") return d.result.error || (d.result.value ? "Returned a typed result" : "Operation finished");
    if (event.kind === "agent_tool_call") return d.name === "run_workflow" ? `run_workflow → ${d.arguments.skill_id}` : d.name === "run_operation" ? `run_operation → ${d.arguments.operation}` : d.name;
  ...
  function render(data) {
    ...
    const steps = new Set(data.events.filter(e=>e.kind === "action_started" && e.data.invocation.includes(":workflow:")).map(e=>e.data.invocation));
    document.getElementById("activity-counts").textContent = `${data.job.model_mode === "live" ? "Live model" : "Simulated model"} · ${data.job.model_calls} model calls · ${calls.size} recorded agent calls · ${steps.size} workflow steps · ${events.length} events shown`;
