...
$launcherPattern = ([regex]::Escape("$Source\src\edge-harness\windows\run-worker.py") + "|" + [regex]::Escape("$Source\src\enterprise\harness\windows\run-worker.py") + "|" + [regex]::Escape("$Source\windows\run-worker.py"))
if (Get-CimInstance Win32_Process | Where-Object { $_.Name -in @('python.exe','pythonw.exe') -and $_.CommandLine -match $launcherPattern }) {
    throw 'A worker process still exists. Stop the idle worker with src/edge-harness/windows/stop-worker.ps1 first.'
}
& $Python -m venv "$Source\.venv"
if ($LASTEXITCODE -ne 0) { throw 'Python virtual environment creation failed.' }
$workerPython = "$Source\.venv\Scripts\python.exe"
& $workerPython -m pip install -c "$Source\requirements.lock" -e "$Source\src\shared" -e "$Source\src\edge-harness"
if ($LASTEXITCODE -ne 0) { throw 'Worker package installation failed.' }
New-Item -ItemType Directory -Force "$Source\runtime" | Out-Null
$launcher = "$Source\src\edge-harness\windows\run-worker.py"
