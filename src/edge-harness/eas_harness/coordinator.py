...
class Coordinator:
    ...
    def build(self, job_id):
        ...
        @tool(args_schema=CapabilityGap)
        def report_capability_gap(capability: str, reason: str) -> dict:
            """Record a missing installed capability for bounded learning/development review. No application access, new tools, deployment or permissions. Not a substitute for completing supported work."""
            return {}
        ...
        if not getattr(settings, "learning_enabled", True):
            allowed.pop("report_capability_gap")
        internal_tools = memory_tools | {"report_capability_gap"}
        ...
        def budget(request, handler):
            ...
            exposed = (
                ...
                else {k: v for k, v in allowed.items() if k in internal_tools | {"select_record"}}
            )
            ...
            while True:
                ...
                if name in internal_tools:
                    if name == "report_capability_gap":
                        value = store.report_capability_gap(job_id, args)
    ...
    def tick(self, job):
        ...
        business_actions = any(
            a["status"] == "executed" and a["name"] != "select_record" for a in self.store.approvals(job_id)
        )
