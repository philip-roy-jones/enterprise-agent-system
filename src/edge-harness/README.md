...
Runs the Deep Agent, skills, server-authorized execution, shadow observation, protected memory and application integrations on an edge machine. The server/frontend is a separate application.

A **skill** is the reusable procedure: instructions, supporting text and an optional constrained graph. `read_skill` retrieves its approved version, `run_skill` executes its graph, and `resume_skill` continues the same interrupted execution. Graph steps reuse registered operations; each step requires its own server authorization while the employee is active. Shadowing permits observation only. The runtime does not execute generated Python.
...
The server's request deadline includes clarification waits and takeover. Expiry prevents further effects. Restart retains authorization records and uncertain-write state; it never authorizes replaying a write.
