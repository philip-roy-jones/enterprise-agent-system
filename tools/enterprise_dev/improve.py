...
from enterprise_dev.improvement_analysis import analyze, patch_library, regression_source
...
def propose(episode, root, output, publish=False):
    ...
    env = {
        ...
        "PYTHONPATH": source_path(checkout),
...
def source_path(checkout):
    """Use only the candidate's application, contract and development sources."""
    return os.pathsep.join(
        str(Path(checkout) / path)
        for path in (
            "src/shared",
            "src/server",
            "src/edge-harness",
