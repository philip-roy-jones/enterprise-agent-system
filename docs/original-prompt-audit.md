...
The PR-based improvement commands, legacy Finance graph, and their dedicated tests were removed on 2026-09-14. References below describe the historical implementation, which remains available in Git history. Current learning uses automatic skill admission and versioned packages.
...
The existing 158-test suite and the latest CI passed, but two additional isolated browser integration checks failed. Both used explicitly simulated staff and model behavior. With a configured 120-second job budget and the synthetic job clock advanced past that budget, the worker continued polling and the job remained `running` at approximately 127 elapsed seconds:
...
| 3. Cyclic role graph | `tools/enterprise_dev/legacy/finance/graph.py`: validate, establish, compare, prepare, save, verify, recover, assist, resume, and complete. Unknown requests also enter supervised discovery. |
