...
def inspect_library(root):
    root = Path(root)
    source = (root / "enterprise/procedures.py").read_text()
    ...
    graph_source = (root / "enterprise/graph.py").read_text()
    ...
    return {
        ...
        "source_hashes": {
            "enterprise/procedures.py": hashlib.sha256(source.encode()).hexdigest(),
            "enterprise/graph.py": hashlib.sha256(graph_source.encode()).hexdigest(),
        },
    }
...
def patch_library(path, analysis):
    ...
    if (
        ...
        != analysis["library"]["source_hashes"]["enterprise/procedures.py"]
    ):
