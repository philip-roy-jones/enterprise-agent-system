...
def test_chat_debug_history_streaming_and_reload(browser_server):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            created = next(event for event in store.events(earlier["id"]) if event["kind"] == "job_created")
            created_row = page.locator(f'[data-message-id="debug-{earlier["id"]}-{created["seq"]}"]')
            created_row.locator(":scope > summary").click()
            expect(created_row.locator(".activity-payload pre")).to_be_visible()
            expect(created_row.locator("details")).to_have_count(0)
            created_row.locator(":scope > summary").click()
